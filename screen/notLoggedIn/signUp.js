...
import {
  ...
  PasswordValidator,
  SignUpValidation,
} from "../../components/validator/credential";
...
export default function SignUp() {
  ...
  async function signUpHandler() {
    ...
    if (SignUpValidation(name, email, confEmail, password, confPassword)) {
      try {
        const token = await createUser(email, password);
        const response = await AddUser(name, email);
        authCtx.authenticate(token, email,name, defaultPreference);
        //console.log(res);
      } catch (e) {
        Alert.alert(
          "Error",
          "Something went wrong, make sure you have entered valid credentials and try again or try again after some time"
        );
      }
    } else {
      Alert.alert(
        "Invalid",
        "Credentials entered are invalid, please try again."
      );
    }
      ...
      <View style={styles.root}>
        <View style={styles.container}>
          ...
          <View style={styles.innerContainer}>
            ...
            />
            {name === "" && (
              <Text style={styles.errorMessage}>Please write your name</Text>
            )}
            <Input
              label={"Email"}
            ...
            />
            {!EmailValidator(email) && emailEntered && (
            ...
            />
            {email !== confEmail && (
              <Text style={styles.errorMessage}>Email is not matching</Text>
            )}
            <Input
              label={"Password"}
              keyboardType="default"
            ...
            {!PasswordValidator(password) && passwordEntered && (
              <Text style={styles.errorMessage}>Password too short</Text>
            )}
            <Input
